...
# 参数
yolo_dir = 'D:/yoloFlaskWang/yolo_date/'  # 你下载YOLO权重的文件路径
CONFIDENCE = 0.5  # 过滤弱检测的最小概率
THRESHOLD = 0.4  # 非最大值抑制阈值


def yolov3(imgPath, name, yolo_dir='D:/yoloFlaskWang/yolo_date/', CONFIDENCE=0.5, THRESHOLD=0.4):
    ...
    # 借助 np.fromfile 和 cv2.imdecode 来实现中文路径的读取
    img = cv.imdecode(np.fromfile(file=imgPath, dtype=np.uint8), cv.IMREAD_COLOR)
    ...
    print("写文件")
    cv.imwrite('static/img/' + name, img)
...
# yolov3(imgPath+name, name, yolo_dir, CONFIDENCE, THRESHOLD)
#
